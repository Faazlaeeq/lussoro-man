...
class ReturnController extends GetxController {
  ...
  getReturnOrders() async {
    returnOrdersModel = await ReturnRepo.getReturnOrders();

    returnOrdersMap.add(returnOrdersModel!);
    ...
    await server
        ...
        .then((response) async {
           response.stream.transform(utf8.decoder).listen((value) {
              ...
              isLoading(false);
              customSnackbar("SUCCESS".tr, 'Return request submitted successfully!'.toString().tr,
                    AppColor.success);
            Get.off(()=> ReturnOrdersScreen());

            } else {
              isLoading(false);
              customSnackbar("ERROR".tr, 'Return request not submitted'.toString().tr,
                    AppColor.error);  
            }
       });
    });
