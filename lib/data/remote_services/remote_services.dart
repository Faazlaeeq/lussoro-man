...
class RemoteServices {
  ...
  Future<Either<String, CategoryWiseProduct>> fetchCategoryWiseProduct({
    ...
    final response = await server
        .postRequest(endPoint: ApiList.categoryWiseProduct, headers: {
  ...
  submitAddress({
    ...
    final response =
        await server.postRequest(endPoint: ApiList.address, headers: {
      "Accept": "application/json, text/plain, /",
      "Access-Control-Allow-Origin": "*",
      "x-api-key": ApiList.licenseCode.toString(),
      'Authorization': 'Bearer ${box.read("token")}',
  ...
  Future<Either<String, ShowAddressModel>> fetchAddress() async {
    final response = await server.getRequest(endPoint: ApiList.showAddress);
    if (response.statusCode == 200) {
      final data = response.data;
  ...
  Future<Either<String, CouponModel>> fetchCoupon() async {
    final response = await server.getRequest(endPoint: ApiList.coupon);
    if (response.statusCode == 200) {
      final data = response.data;
